/* Radius Health Guides — Mode detail page */
const { useState: useStateM } = React;

function ModePage({ modeId, duration, onBack }) {
  const mode = window.MODES[modeId];
  const phases = mode.phases[duration];
  const PLAN_INFO = {
    accelerator: { label: 'Accelerator · 45-day', accent: '#E60023', days: '45 days', cadence: '4× / week · 15 min', eyebrow: '45-Day · Accelerator' },
    silver:      { label: 'Silver · 60-day',      accent: '#B5B5BC', days: '60 days', cadence: '2× / week · 15 min', eyebrow: '60-Day · Silver' },
    gold:        { label: 'Gold · 90-day',        accent: '#C9A862', days: '90 days', cadence: '2× / week · 15 min', eyebrow: '90-Day · Gold' },
  };
  const planInfo = PLAN_INFO[duration];
  const planLabel = planInfo.label;
  const planAccent = planInfo.accent;
  ...
  return (
    <div>
      ...
      <section style={{ position: 'relative', minHeight: '72vh', overflow: 'hidden', borderBottom: '1px solid var(--border-subtle)' }}>
        ...
        <div style={{ position: 'relative', maxWidth: 'var(--page-max)', margin: '0 auto', padding: '120px 28px 96px' }}>
          <button onClick={onBack} style={{
            ...
            color: 'var(--fg-2)', padding: '8px 16px', fontSize: 11,
          ...
          <div style={{ display: 'flex', gap: 48, marginTop: 48, flexWrap: 'wrap', borderTop: '1px solid var(--border-subtle)', paddingTop: 32 }}>
            ...
            <Stat label="Cadence" value={planInfo.cadence} />
            <Stat label="Plan" value={planLabel} accent={planAccent} />
            <Stat label="Duration" value={planInfo.days} />
          </div>
        </div>
      </section>
      ...
      <Section eyebrow="Mechanism" title="How this mode works">
        ...
        <p style={{ fontSize: 15, color: 'var(--fg-3)', marginTop: 24, maxWidth: '70ch', fontStyle: 'italic' }}>
          "{mode.mantra}"
        </p>
      </Section>
      ...
      <Section eyebrow="The Session" title="Session-day protocol" background="var(--bg-raised)">
        ...
        <div style={{ marginTop: 2, background: 'var(--bg-card)', padding: '32px', display: 'grid', gridTemplateColumns: '160px 1fr', gap: 32, alignItems: 'center', borderTop: '1px solid var(--accent)' }}>
          ...
          <div>
            ...
            <a href="https://www.amazon.com/s?k=earth+harmony+methylene+blue" target="_blank" rel="noopener"
      ...
      {/* Phases — the heart of the plan */}
      <Section eyebrow={planInfo.eyebrow} title="Phased plan">
      ...
      <Section eyebrow="Daily Rhythm" title="Your day, structured" background="var(--bg-raised)">
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 2, background: 'var(--border-subtle)' }}>
          {window.SHARED.dailyRhythm.map((block, i) => (
            <div key={i} style={{ background: 'var(--bg-card)', padding: '32px 28px', borderTop: '1px solid transparent' }}>
      ...
      {/* Nutrition / Exercise / Supplements — mode-specific */}
      ...
      {/* Full stack — shared. Custom header: logo sits flush next to title. */}
      <section style={{ background: 'var(--bg-raised)', padding: '96px 28px' }}>
        <div style={{ maxWidth: 'var(--page-max)', margin: '0 auto' }}>
          ...
          {/* Title + logo to the right, flush together */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 40, flexWrap: 'wrap' }}>
            ...
            <img
              ...
              style={{
                ...
                width: 'auto',
                flex: '0 0 auto',
                marginLeft: -12, // optical: PNG has whitespace padding
              }}
            />
          </div>
          ...
          {/* Supplement table */}
          <div style={{ display: 'grid', gap: 2, background: 'var(--border-subtle)' }}>
            {window.SHARED.supplements.map((s, i) => (
              <div key={i} style={{ background: 'var(--bg-card)' }}>
                <div style={{ padding: '24px 28px', display: 'grid', gridTemplateColumns: '2fr 1.5fr 3fr', gap: 32, alignItems: 'start' }}>
                  <div>
                ...
                {/* Expanded capsule grid for the Super Stack */}
                {s.items && (
                  <div style={{ borderTop: '1px solid var(--border-subtle)', padding: '8px 28px 28px' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 2, background: 'var(--border-subtle)' }}>
                      {s.items.map((c, j) => {
                        const isPM = /PM/i.test(c.time);
                        return (
                          <div key={j} style={{
                            ...
                            <div style={{
                              ...
                              <img src={c.image} alt={c.name} style={{ maxWidth: '90%', maxHeight: '90%', objectFit: 'contain', display: 'block' }} />
                            </div>
                            <div>
                              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 4 }}>
                                <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--accent)', fontWeight: 700 }}>{c.n}</span>
                                <span style={{ fontFamily: 'var(--font-display)', fontSize: 14, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '-0.005em', lineHeight: 1.2 }}>{c.name}</span>
                              </div>
                              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--fg-3)', marginBottom: 8 }}>{c.dose}</div>
                              <div style={{
                              ...
                              <div style={{ fontSize: 12, color: 'var(--fg-2)', lineHeight: 1.5 }}>{c.role}</div>
                            </div>
                          </div>
                        );
                      })}
      ...
      <Section eyebrow="Practice Library" title="The practices underneath">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 2, background: 'var(--border-subtle)' }}>
          {Object.entries(window.SHARED.practices).map(([k, p]) => (
            <div key={k} style={{ background: 'var(--bg-card)', padding: '28px 24px', display: 'flex', flexDirection: 'column' }}>
              ...
              {p.video && (
                <div style={{ marginTop: 16, paddingTop: 14, borderTop: '1px solid var(--border-subtle)' }}>
                  ...
                  <div style={{ fontSize: 10, color: 'var(--fg-3)', marginTop: 4, fontStyle: 'italic' }}>
                    {p.video.credit}
                  </div>
                </div>
              )}
        ...
        <div style={{ marginTop: 2, background: 'var(--bg-card)', padding: '28px 24px' }}>
          <RedRule w={32} style={{ marginBottom: 14 }} />
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 22, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '-0.01em', marginBottom: 14 }}>{window.SHARED.nutritionBase.title}</div>
          <ul style={{ margin: 0, padding: 0, listStyle: 'none' }}>
            {window.SHARED.nutritionBase.lines.map((l, i) => (
              <li key={i} style={{ color: 'var(--fg-2)', fontSize: 14, marginBottom: 10, lineHeight: 1.5 }}>{l}</li>
            ))}
          </ul>
        </div>
      ...
      <section style={{ background: 'var(--bg-raised)', borderTop: '1px solid var(--accent)', padding: '80px 28px 96px' }}>
        <div style={{ maxWidth: 'var(--page-max)', margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 48 }}>
            <div>
              <p style={{ color: 'var(--fg-2)', fontSize: 15, lineHeight: 1.6 }}>{window.SHARED.disclaimer.short}</p>
            ...
            <div>
              ...
              <ul style={{ margin: 0, padding: 0, listStyle: 'none' }}>
                {window.SHARED.disclaimer.contraindications.map((c, i) => (
